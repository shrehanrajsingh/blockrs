...
namespace rs::block
{
BlocknetServer::BlocknetServer () : _Server_Base () {}
...
BlocknetServer::setup_socket ()
{
  ...
  if (listen (fd, get_max_clients ()))
    throw std::runtime_error ("listen failed");

  dbg ("Blockchain Server running on " << BKRS_SERVER_URL << ':'
                                       << get_port ());
  std::cout.flush ();
}

void
BlocknetServer::handle_client (int client_fd)
{
  ...
  while (get_is_running ())
    {
      ...
      {
        std::lock_guard<std::mutex> lock (cout_mutex);
        dbg ("New client connected! [client_fd: " << client_fd << "]");
      }

      std::thread t (&BlocknetServer::handle_client, this, client_fd);
...
void
BlocknetServer::close ()
{
  if (fd != -1)
    {
      ::close (fd);
      fd = -1;
      dbg ("BlocknetServer::close(): Server shutting down...");
    }
}

int
BlocknetServer::get_port ()
{
  return this->_Server_Base::get_port ();
}

void
BlocknetServer::set_port (int _P)
{
  this->_Server_Base::set_port (_P);
}

int
BlocknetServer::get_max_clients ()
{
  return this->_Server_Base::get_max_clients ();
}

void
BlocknetServer::set_max_clients (int _C)
{
...
<html>
<head>
<title>BlockRS::BlocknetServer</title>
<style>
:root {
--bg-color: #f8f8f8;
--text-color: #333;
